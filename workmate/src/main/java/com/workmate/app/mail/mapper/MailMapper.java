...
public interface MailMapper {
 ...
 // 특정 사용자  폴더 ID에 해당하는 메일 목록 조회
    List<MailVO> selectMailsByFolderId(@Param("userNo") int userNo,
  ...
  //특정 폴더(FOLDER_ID) 안에 있는 메일들만 조회
    List<MailVO> selectMailListByFolderId(@Param("userNo") int userNo,
            @Param("folderId") int folderId);
  //특정폴더이름 넘기기(내가만든 폴더)
 ...
 // 공용 페이징 메일 조회
    List<MailVO> selectMailListPaging(@Param("userNo") int userNo,
                                       @Param("folderId") int folderId,
    ...
    //임시보관 
    List<MailVO> selectDraftMails(@Param("userNo") int userNo);
}
